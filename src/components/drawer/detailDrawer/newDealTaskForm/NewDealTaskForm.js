...
const NewDealTaskForm = ({ idNegocio, dataNegocio, closeDrawer }) => {
  ...
  const URL = `${PROTOCOL}//${HOSTNAME}:${PORTENDPOINT}`;
  ...
  const [newTareaResolver] = useMutation(NEW_TAREA_DE_NEGOCIO, {
    onCompleted: () => {
      OpenNotification(
      ...
      //refetch query data del negocio
      queryPollDealContent.initial(2000);
      queryPollTareas.initial(2000);
      setTimeout(() => {
        queryPollDealContent.close();
        queryPollTareas.close();
      }, 3000);

      setNoteContent("");
      //refetch query para la tabla de tareas
  ...
  const onFinish = (v) => {
    ...
    newTareaResolver({
      variables: {
        ...
        idUsuario: idUser,
        idCliente: dataNeg.cli_id ? Number(dataNeg.cli_id) : null,
        inputAdjunto: objetoUpload,
        idContacto: dataNeg.con_id ? Number(dataNeg.con_id) : null,
        idUsuarioAsignado: v.usuarioAsignado ? v.usuarioAsignado : idUser,
      },
    }).then((tarea) => {
            ...
            <Form.Item name="upload">
              ...
              >
                <p className="ant-upload-drag-icon">
                  <InboxOutlined style={{ color: "#00b33c" }} />
                </p>
...
export default NewDealTaskForm;
